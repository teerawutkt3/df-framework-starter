<nested:define id="_condition" property="searchCondition3" type="com.depthfirst.framework.search.SearchCondition"/>
<nested:define id="_result" property="searchResult3" type="com.depthfirst.framework.search.SearchResult"/>
<nested:hidden property="searchCondition3.position" /><%!
...
%><table align="center" border="0" width="95%"><tr class="trcursor">
  ...
  %><select name="searchCondition3.pageSize" onchange="doGoto3(0)"><% for(int _p=0; _p<_perpages.length; _p++) {
  %><option value="<%= _perpages[_p] %>"<%= _psize==_perpages[_p]?" selected":"" %>>หน้าละ <%= _perpages[_p] %> รายการ</option><% } %></select><% } %>
  &nbsp;<% if(_prev || _next) { %>
    หน้าที่ <select onchange="doGoto3(this.selectedIndex*<%= _psize %>)">
  ...
  <td width="30%" align="right" class="noprint" nowrap="nowrap">&nbsp;<% if(_prev || _next) { %>
    <% if(_prev) { %>
    <a href="javascript:doGoto3(0)"><img border="0" src="<%= request.getContextPath() %>/images/cursor/i.p.firstpg.gif" /></a>
    <a href="javascript:doGoto3(<%= _pos-_psize %>)"><img border="0" src="<%= request.getContextPath() %>/images/cursor/i.p.prevpg.gif" /></a>
    <% }else { %>
    ...
    <% } %>
    <% if(_next) { %>
    <a href="javascript:doGoto3(<%= _pos+_psize %>)"><img border="0" src="<%= request.getContextPath() %>/images/cursor/i.p.nextpg.gif" /></a>
    <a href="javascript:doGoto3(<%= _size%_psize>0?(_size-(_size%_psize)):(_size-_psize) %>)"><img border="0" src="<%= request.getContextPath() %>/images/cursor/i.p.lastpg.gif" /></a>
    <% }else { %>
